...
// minimize the number of variables used because of space and memory considerations
            ...
            if (value == true) {
                ...
                svContext.setAttribute("user", userClass);
...
//                svContext.setAttribute("UserID", userClass.getIduser());
                ...
                testLecturer.setUser_iduser(userClass.getIduser());
                LecturerDao lexeme = new LecturerDao();
                try {
                    // The person working on this module should collect the following list and let it display as per the given day and hour
                    List<Lecture> lexlist = lexeme.getAllLecturesByLecturer(testLecturer);
                    svContext.setAttribute("lexlist", lexlist);
                } catch (SQLException ex) {
                    Logger.getLogger(LoginServlet.class.getName()).log(Level.SEVERE, null, ex);
                }
                switch (accesslevel) {
                    ...
                    case 0:
                        RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/deanView.jsp");
                        rd.forward(request, response);
                        //response.sendRedirect("deanView.jsp");
                        break;
                    case 1:
                        RequestDispatcher rd1 = request.getRequestDispatcher("/WEB-INF/dashboard.jsp");
                        rd1.forward(request, response);
                        //response.sendRedirect("dashboard.jsp");
                        break;
                    case 2:
                        RequestDispatcher rd2 = request.getRequestDispatcher("/WEB-INF/hodView.jsp");
                        rd2.forward(request, response);
                        //response.sendRedirect("hodView.jsp");
                        break;
                    default:
                        RequestDispatcher rd3 = request.getRequestDispatcher("/WEB-INF/lecturerView.jsp");
